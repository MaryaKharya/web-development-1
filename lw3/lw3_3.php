<?php
  ...
  if (preg_match_all("/[A-Z]/", $text) > 0)
    $protection = $protection + ((strlen($text) - preg_match_all("/[A-Z]/", $text))*2); //Если есть ЗАГЛАВНЫЕ буквы добавляет (len-n)*2
  if (preg_match_all("/[a-z]/", $text) > 0)
    $protection = $protection + ((strlen($text) - preg_match_all("/[a-z]/", $text))*2); //Если есть СТРОЧНЫЕ буквы добавляет (len-n)*2
  ...
  $arr = str_split($text);                   //Создаёт из строки явный массив символов
